...
> [!NOTE]
> 외부 진입 커맨드는 `autoresearch` 하나만 사용합니다. `autoresearch init`은 의도적으로 지원하지 않습니다.

## 첫 실행

1. 빈 프로젝트에서 `autoresearch`를 실행합니다.
2. `.autoresearch/config.yaml`이 없으면 setup wizard가 자동으로 시작됩니다.
3. wizard가 설정과 스캐폴드를 만들고, Semantic Scholar 키를 저장한 뒤 대시보드로 진입합니다.
