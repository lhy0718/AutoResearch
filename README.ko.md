...
용어 정리:
- Workflow mode: 현재 9개 노드 실행 구조는 `agent_approval`입니다.
- Approval mode: `minimal`(기본값)은 일반 승인 경계를 자동 해소하고, `manual`은 모든 승인 경계에서 멈춥니다.
- Autonomy preset: `/agent overnight`는 별도의 안전한 야간 자율 실행 정책입니다. 세 번째 workflow mode가 아닙니다.

`agent_approval` workflow mode 안에서 기본 approval mode는 `minimal`이므로, 성공한 노드는 자동으로 다음 단계로 넘어가고 review 결과도 자동으로 `write_paper` 또는 지원되는 backtrack으로 적용됩니다. 현재 사람이 꼭 판단해야 하는 대표적인 지점은 `analyze_results` 안에 있습니다. objective metric을 구체적인 수치 신호에 연결하지 못해 clarification이 필요한 경우와, hypothesis reset 추천의 confidence가 충분히 높지 않아 auto-executable로 표시되지 않는 경우입니다. 예전처럼 각 노드마다 멈추고 review backtrack도 `/approve`로 진행하고 싶다면 `workflow.approval_mode: manual`로 바꾸면 됩니다.
...
flowchart LR
    ...
    TUI --> Approve["자동 전이 또는 /approve (manual approval mode)"]
    Web --> Approve
...
| `/approve` | 멈춘 현재 노드를 승인 (주로 approval mode가 `manual`일 때 사용) |
| `/retry` | 현재 노드 재시도 |
| `/settings` | 기본 설정 수정 |
| `/quit` | 종료 |
