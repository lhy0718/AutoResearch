...
| `OPENAI_API_KEY` | Когда провайдер или режим PDF — `api` | Выполнение моделей через OpenAI API |
| Вход в Codex CLI | Когда провайдер или режим PDF — `codex` | Используется ваша локальная сессия Codex |
